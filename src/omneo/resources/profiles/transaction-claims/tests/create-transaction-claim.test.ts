...
import { TransactionClaim, RequestClaimTransaction } from '@types'
...
describe('Profile Transaction claim create', () => {
  test('SDK Profile Create Transaction claim', async () => {
      ...
      items: [
        {
          ...
          name: getRandomString('sdk_unit_Profile_Transaction_claim_create_item'),
          price_current: 49.99,
          price_sell: 49.99,
          quantity: 1
        }
    ...
    const response = await simpleOmneoRequest('POST', '/transactions', payload).catch((err) => {
      console.error('SDK create transaction claim, transaction created failed:', err)
      throw new Error('SDK create transaction claim, transaction created failed')
    })
    ...
    const claimRes: TransactionClaim = await omneo.profiles.transactionClaims.create(testProfileID, claimInput)
