...
describe('Get Profile List Item', () => {
  test('SDK Get Profile List Item', async () => {
    // Create List Definition
    const payload = {
      name: getRandomString('sdk_list_definition_for_get_list_item'),
      handle: getRandomString('sdk_list_definition_for_get_list_item'),
      type: 'gift_registry',
      is_published: true,
      allow_quantity: true
    }
    ...
    const payload2 = {
      ...
      name: getRandomString('sdk_list_name_for_get_list_item')
    }
    ...
    const listItem = await omneoClient.profiles.lists.items.get(testProfileID, listId, listItemId).catch((err: any) => {
      console.error('SDK Get profile list item failed:', err)
      throw new Error('SDK Get profile list item failed')
    })
