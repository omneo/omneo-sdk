...
describe('Update Profile List Item', () => {
  test('SDK Update Profile List Item', async () => {
    // Create List Definition
    const payload = {
      name: getRandomString('sdk_list_definition_for_update_list_item'),
      handle: getRandomString('sdk_list_definition_for_update_list_item'),
      type: 'gift_registry',
      is_published: true,
      allow_quantity: true
    }
    ...
    const payload2 = {
      ...
      name: getRandomString('sdk_list_name_for_update_list_item')
    }
    ...
    const listItem = await omneoClient.profiles.lists.items.update(testProfileID, listId, listItemId, payload4 as any).catch((err: any) => {
      console.error('SDK Update profile list item failed:', err)
      throw new Error('SDK Update profile list item failed')
    })
