...
describe('Create Profile List Custom Item', () => {
  test('SDK Create Profile List Custom Item', async () => {
    // Create List Definition
    const payload = {
      name: getRandomString('sdk_list_definition_for_create_list_custom_item'),
      handle: getRandomString('sdk_list_definition_for_create_list_custom_item'),
      type: 'gift_registry',
      is_published: true,
      allow_quantity: true,
    ...
    const payload2 = {
      ...
      name: getRandomString('sdk_list_name_for_create_list_custom_item')
    }
    ...
    // Create List Item
    const payload3 = {
      name: getRandomString('sdk_list_custom_item_name'),
      description: getRandomString('sdk_list_custom_item_description'),
    ...
    const listItem = await omneoClient.profiles.lists.items.custom(testProfileID, listId, payload3 as any).catch((err: any) => {
      console.error('SDK Create profile list custom item failed:', err)
      throw new Error('SDK Create profile list custom item failed')
    })
