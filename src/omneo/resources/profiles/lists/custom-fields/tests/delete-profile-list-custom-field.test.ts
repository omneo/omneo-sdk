...
describe('Delete Profile List Custom Field', () => {
  test('SDK Delete Profile List Custom Field', async () => {
    ...
    await omneoClient.profiles.lists.customFields.delete(testProfileID, listId, payload3.namespace, payload3.handle).catch((err: any) => {
      ...
      throw new Error('SDK Delete profile list custom field failed')
    })
...
afterAll(async () => {
  if (FAILED_DELETE_LIST_CUSTOM_FIELDS.length > 0) {
    for (const { namespace, handle, listId } of FAILED_DELETE_LIST_CUSTOM_FIELDS) {
      console.log('Cleaning up SDK Delete Custom Field with namespace, handle', namespace, handle, listId)
      ...
      if (deleteResponse.status === 204) {
        console.log(`SDK Delete Custom Field with namespace ${namespace} and handle ${handle} deleted`)
      } else {
        console.log(`Failed to delete Delete Custom Field with namespace ${namespace} and handle ${handle}`, deleteResponse)
      }
    }
  }
