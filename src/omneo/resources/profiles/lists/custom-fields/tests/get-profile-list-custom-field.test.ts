...
describe('Get Profile List Custom Field', () => {
  test('SDK Get Profile List Custom Field', async () => {
    ...
    const listProfileCustomAttribute = await omneoClient.profiles.lists.customFields.get(testProfileID, listId, payload3.namespace, payload3.handle).catch((err: any) => {
      console.error('SDK Get profile list custom field failed:', err)
      throw new Error('SDK Get profile list custom field failed')
    })
...
afterAll(async () => {
  if (CREATED_LIST_CUSTOM_FIELDS.length > 0) {
    for (const { namespace, handle, listId } of CREATED_LIST_CUSTOM_FIELDS) {
      console.log('Cleaning up SDK Get Custom Field with namespace, handle', namespace, handle, listId)
      ...
      if (deleteResponse.status === 204) {
        console.log(`SDK Get Custom Field with namespace ${namespace} and handle ${handle} deleted`)
      } else {
        console.log(`Failed to delete Get Custom Field with namespace ${namespace} and handle ${handle}`, deleteResponse)
      }
    }
  }
