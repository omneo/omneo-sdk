...
describe('Redeem Profile Credit', () => {
  test('SDK Redeem Profile Credit', async () => {
    const payload = {
      name: getRandomString('sdk_name_for_profile_redeem_credit'),
      handle: getRandomString('sdk_handle_for_profile_redeem_credit'),
      is_published: true,
      is_extendable: true,
      is_assignable: true,
      is_releasable: true,
    ...
    const payload2 = {
      ...
      message: getRandomString('sdk_message_for_profile_redeem_credit'),
      value_initial: 100,
    ...
    const redemption: Redemption = await omneoClient.profiles.credits.redeem(testProfileID, creditId, payload3)
...
afterAll(async () => {
  if (CREATED_CREDIT_IDS.length > 0) {
    for (const id of CREATED_CREDIT_IDS) {
      ...
      if (deleteResponse.status === 204) {
        console.log(`SDK Redeem Profile Credit, Credit ID ${id} deleted`)
      } else {
  ...
  if (CREATED_CREDIT_DEFINITION_IDS.length > 0) {
    for (const id of CREATED_CREDIT_DEFINITION_IDS) {
      ...
      if (deleteResponse.status === 204) {
        console.log(`SDK Redeem Profile Credit, Credit Definition ID ${id} deleted`)
      } else {
