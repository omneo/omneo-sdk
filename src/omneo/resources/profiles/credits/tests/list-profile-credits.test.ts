...
describe('List Profile Credits', () => {
  test('SDK List Profile Credits', async () => {
    const payload = {
      name: getRandomString('sdk_name_for_profile_list_credits'),
      handle: getRandomString('sdk_handle_for_profile_list_credits'),
      is_published: true,
      is_extendable: true,
      is_assignable: true,
      is_releasable: true,
    ...
    const payload2 = {
      ...
      message: getRandomString('sdk_message_for_profile_list_credits'),
      value_initial: 100,
    ...
    const params = {}
    const { data: credits }: CreditResponse = await omneoClient.profiles.credits.list(testProfileID, params)
...
afterAll(async () => {
  if (CREATED_CREDIT_IDS.length > 0) {
    for (const id of CREATED_CREDIT_IDS) {
      ...
      if (deleteResponse.status === 204) {
        console.log(`SDK List Profile Credits, Credit ID ${id} deleted`)
      } else {
  ...
  if (CREATED_CREDIT_DEFINITION_IDS.length > 0) {
    for (const id of CREATED_CREDIT_DEFINITION_IDS) {
      ...
      if (deleteResponse.status === 204) {
        console.log(`SDK List Profile Credits, Credit Definition ID ${id} deleted`)
      } else {
