...
describe('Create Credit Definition', () => {
  test('SDK Create Credit Definition', async () => {
    const payload = {
      name: getRandomString('sdk_name_for_create_credit_definition'),
      handle: getRandomString('sdk_handle_for_create_credit_definition'),
      is_published: true,
      type: 'gift_card',
      ...
      short_description: 'sdk_short_desc_for_create_credit_definition',
      description: 'sdk_desc_for_create_credit_definition',
      long_description: 'sdk_long_desc_for_create_credit_definition',
      value: 100
    }
  ...
  if (CREATED_CREDIT_DEFINITION_IDS.length > 0) {
    for (const id of CREATED_CREDIT_DEFINITION_IDS) {
      ...
      if (deleteResponse.status === 204) {
        console.log(`SDK Create Credit Definition ID ${id} deleted`)
      } else {
