...
import { Reward, RequestUpdateReward } from '@types'
...
describe('Reward update', () => {
  test('SDK Reward update', async () => {
    ...
    const updatedPayload: RequestUpdateReward = {
      value_initial: Math.floor(Math.random() * 1000),
      value_remaining: Math.floor(Math.random() * 1000)
    }
    const targetReward: Reward = await omneo.rewards.update(rewardResponse.data.id, updatedPayload).catch((err) => {
      console.error('SDK Reward definition created failed:', err)
      throw new Error('SDK Reward definition created failed')
    })
    expect(targetReward.value_initial).toBe(updatedPayload.value_initial)
    expect(targetReward.value_remaining).toBe(updatedPayload.value_remaining)
