...
describe('ID Get Profile List Custom Fields', () => {
  testWithIDData('ID SDK Get Profile List Custom Fields', async ({ IDData }) => {
    ...
    const listCustomField = await IDClient.profile.lists.customFields.get(listId, payload3.namespace, payload3.handle).catch((err: any) => {
      console.error('ID SDK Get profile list custom fields failed:', err)
      throw new Error('ID SDK Get profile list custom fields failed')
    })
