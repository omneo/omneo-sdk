...
describe('ID Profile Tiers - List', () => {
  testWithIDData('ID SDK can list profile tiers', async ({ IDData }) => {
    ...
    if (existingDefinitions.data && existingDefinitions.data.length > 0) {
      ...
      const tierDefinitionPayload = {
        name: getRandomString('sdk_unit_test_id_list_profile_tier'),
        handle: getRandomString('sdk_unit_test_id_list_profile_tier'),
    ...
    const tiers: TierProgress = await IDClient.profile.tiers.list().catch((err) => {
      console.error('ID SDK List Profile Tiers failed:', err)
      throw new Error('ID SDK List Profile Tiers failed')
    })
