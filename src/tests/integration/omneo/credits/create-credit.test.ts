...
describe('Create Credit', () => {
  test('SDK Create Credit', async () => {
    const payload = {
      name: getRandomString('sdk_name_for_create_credit'),
      handle: getRandomString('sdk_handle_for_create_credit'),
      is_published: true,
      is_extendable: true,
      is_assignable: true,
      is_releasable: true,
    ...
    const payload2 = {
      ...
      message: getRandomString('sdk_message_for_create_credit'),
      value_initial: 100,
      credit_number: getRandomDigitString(10),
      security_code: '1113'
    }

    const creditRes: Credit = await omneoClient.credits.create(payload2)
    CREATED_CREDIT_IDS.push(creditRes.id)
  ...
  if (CREATED_CREDIT_IDS.length > 0) {
    for (const id of CREATED_CREDIT_IDS) {
      ...
      if (deleteResponse.status === 204) {
        console.log(`SDK Create Credit ID ${id} deleted`)
      } else {
