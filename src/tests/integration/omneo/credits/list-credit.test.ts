...
describe('List Credits', () => {
  test('SDK List Credits', async () => {
    const payload = {
      name: getRandomString('sdk_name_for_list_credits'),
      handle: getRandomString('sdk_handle_for_list_credits'),
      is_published: true,
      is_extendable: true,
      is_assignable: true,
      is_releasable: true,
    ...
    const payload2 = {
      ...
      message: getRandomString('sdk_message_for_list_credits'),
      value_initial: 100,
    ...
    const { data: credits }: CreditResponse = await omneoClient.credits.list(params)
  ...
  if (CREATED_CREDIT_IDS.length > 0) {
    for (const id of CREATED_CREDIT_IDS) {
      ...
      if (deleteResponse.status === 204) {
        console.log(`SDK List Credit ID ${id} deleted`)
      } else {
