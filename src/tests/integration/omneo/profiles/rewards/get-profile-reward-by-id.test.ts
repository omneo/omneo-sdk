...
describe('Profile Get Reward', () => {
  test('SDK Profile Get Reward by ID', async () => {
    ...
    const profileReward: Reward = await omneo.profiles.rewards.get(rewardPayload.profile_id, targetReward.id).catch((err) => {
      console.error('SDK Get profile reward by ID failed:', err)
      throw new Error('SDK Get profile reward by ID failed')
    })
