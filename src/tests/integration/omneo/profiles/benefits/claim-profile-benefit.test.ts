...
const buildDefinitionPayload = (): BenefitDefinitionInput => ({
  name: getRandomString('sdk_unit_test_profile_benefit_claim_name'),
  handle: getRandomString('sdk_unit_test_profile_benefit_claim_handle'),
  period: 30,
  is_published: true,
  is_assignable: true,
  is_claimable: true,
...
describe('Claim Profile Benefit', () => {
  test('SDK Claim Profile Benefit', async () => {
    ...
    expect(benefit.profile_id).toBe(testProfileID)
...
afterAll(async () => {
  if (CREATED_BENEFIT_IDS.length > 0) {
    for (const id of CREATED_BENEFIT_IDS) {
      ...
      if (deleteResponse.status === 204) {
        console.log(`SDK Claim Profile Benefit, Benefit ID ${id} deleted`)
      } else {
  ...
  if (CREATED_BENEFIT_DEFINITION_IDS.length > 0) {
    for (const id of CREATED_BENEFIT_DEFINITION_IDS) {
      ...
      if (deleteResponse.status === 204) {
        console.log(`SDK Claim Profile Benefit, Benefit Definition ID ${id} deleted`)
      } else {
