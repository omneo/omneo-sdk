...
describe('Claim Redeem Profile Benefit', () => {
  test('SDK Claim Redeem Profile Benefit', async () => {
    ...
    const redemption = await omneoClient.profiles.benefits.claimRedeem(testProfileID, claimInput)
...
afterAll(async () => {
  if (CREATED_BENEFIT_IDS.length > 0) {
    for (const id of CREATED_BENEFIT_IDS) {
      ...
      if (deleteResponse.status === 204) {
        console.log(`SDK Claim Redeem Profile Benefit, Benefit ID ${id} deleted`)
      } else {
  ...
  if (CREATED_BENEFIT_DEFINITION_IDS.length > 0) {
    for (const id of CREATED_BENEFIT_DEFINITION_IDS) {
      ...
      if (deleteResponse.status === 204) {
        console.log(`SDK Claim Redeem Profile Benefit, Benefit Definition ID ${id} deleted`)
      } else {
