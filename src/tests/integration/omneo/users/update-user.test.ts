...
describe('Users update', () => {
  test('SDK can update a user', async () => {
    const testHandle = getHandle()
    const password = randomString(32)
    const payload = {
      name: 'SDK Unit Test Update User',
    ...
    const user = await omneo.users.update(createdData.id, { name: randomUpdatedString })
    expect(user.name).toEqual(randomUpdatedString)
  })
})
