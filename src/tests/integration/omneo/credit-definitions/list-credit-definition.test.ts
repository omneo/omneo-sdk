...
import { CreditDefinition, CreditDefinitionResponse } from '@types'
...
describe('List Credit Definitions', () => {
  test('SDK List Credit Definitions', async () => {
    const payload = {
      name: getRandomString('sdk_name_for_list_credit_definitions'),
      handle: getRandomString('sdk_handle_for_list_credit_definitions'),
      is_published: true,
      type: 'gift_card',
      ...
      short_description: 'sdk_short_desc_for_list_credit_definitions',
      description: 'sdk_desc_for_list_credit_definitions',
      long_description: 'sdk_long_desc_for_list_credit_definitions',
      value: 100
    }
    ...
    const { data: creditDefinitions }: CreditDefinitionResponse = await omneoClient.creditDefinitions.list(params)
    expect(creditDefinitions).toBeDefined()

    const targetDefinition: CreditDefinition = creditDefinitions[0]
  ...
  if (CREATED_CREDIT_DEFINITION_IDS.length > 0) {
    for (const id of CREATED_CREDIT_DEFINITION_IDS) {
      ...
      if (deleteResponse.status === 204) {
        console.log(`SDK List Credit Definition ID ${id} deleted`)
      } else {
