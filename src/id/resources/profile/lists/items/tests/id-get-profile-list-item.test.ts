...
describe('ID Get Profile List Item', () => {
  testWithIDData('ID SDK Get Profile List Item', async ({ IDData }) => {
    const { tokenData } = IDData
    // Create List Definition
    const payload = {
      name: getRandomString('id_sdk_list_definition_name_for_get_list_item'),
      handle: getRandomString('id_sdk_list_definition_handle_for_get_list_item'),
      type: 'gift_registry',
      is_published: true,
      allow_quantity: true,
    ...
    const payload2 = {
      ...
      name: getRandomString('id_sdk_list_name_for_get_list_item')
    }
    ...
    const listItem = await IDClient.profile.lists.items.get(listId, listItemId).catch((err: any) => {
      console.error('ID SDK Get profile list item failed:', err)
      throw new Error('ID SDK Get profile list item failed')
    })
