...
describe('ID Create Profile List Item', () => {
  testWithIDData('ID SDK Create Profile List Item', async ({ IDData }) => {
    const { tokenData } = IDData
    // Create List Definition
    const payload = {
      name: getRandomString('id_sdk_list_definition_name_for_create_list_item'),
      handle: getRandomString('id_sdk_list_definition_handle_for_create_list_item'),
      type: 'gift_registry',
      is_published: true,
      allow_quantity: true,
